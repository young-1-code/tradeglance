...
| 2026-06-15 | v1.1 | Phase 3 编码完成 | tg-decision-agent（LLM 决策层，8+1 tests）+ tg-mock-order-engine（模拟撮合+虚拟账户+A股规则，13 tests）由 codex 实现并提交；信号→决策→执行全链路代码闭环；ExecutionHandler 跨 backtest/mock 一致；全服务 E2E 联调待 Phase 4 compose |
...
| 3 | tg-decision-agent / tg-mock-order-engine | ✅ 完成 | 🚧 已实现+tests 绿（b1efdb8/51790fe）；全链路 E2E 联调待 Phase 4 compose |
